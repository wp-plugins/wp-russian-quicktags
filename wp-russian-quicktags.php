...
function wrq_change_comment_form_defaults($default) {
  ...
  $purl = plugins_url(); 
  $url = $purl . '/wp-russian-quicktags/scripts.php'; 
  ...
  return $default; }
add_filter('comment_form_defaults', 'wrq_change_comment_form_defaults'); 
...
function wp_russian_quicktags() {
	...
	?>
	<div id="comment_quicktags">
	<script src="<?php echo $purl . '/wp-russian-quicktags/scripts.php'; ?>" type="text/javascript"></script>
	<script type="text/javascript">edToolbar();</script>
	</div>
<?php } ?>